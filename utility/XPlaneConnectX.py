...
"""
Credit to Marc Schlichting at the Stanford Intelligent Systems Laboratory (SISL) for developing this plugin
https://github.com/sisl/XPlaneConnectX
"""

class XPlaneConnectX():
    def __init__(self,ip:str='127.0.0.1',port:int=49000, sim_mode:SimMode = None) -> None:
        ...
        Args:
            ...
            sim_mode (None or SimMode class): If provided, XPCX sends and receives DREFs from the SimMode class instead of X-Plane
        ...
        self.cached_dref_values = {} # For caching last value of each dataref received.
    ...
    def getDREF(self, dref:str) -> float:
        ...
        """
        if self.sim_mode is not None:
            value = self.sim_mode.getDREF(dref)
            ...
            try:
                ...
                self.cached_dref_values[dref] = value
                if idx_received != idx:
                ...
                cached_value = self.cached_dref_values[dref]
                print(f'ERROR getting dref {dref}. Using cached value {cached_value}')
                return cached_value
        
        return value
    
    def sendDREF(self, dref:str, value:float, importance=0) -> None:
        ...
        """
        #print(f'DREF {dref}')
        max_tries = 1 if importance == 0 else 3 if importance == 1 else 7

        success = False
        num_tries = 0
        while (not success) or (num_tries < max_tries):
            num_tries += 1
            try:
                if self.sim_mode is not None:
                    self.sim_mode.sendDREF(dref, value)
                else:
                    msg = struct.pack('<4sxf500s', b'DREF', value, dref.encode('UTF-8'))
                    self.sock.sendto(msg, (self.ip, self.port))
                success = True
            except Exception as e:
                print(f'ERROR: failed to send DREF to {dref}')
                print(e)
                success = False
    ...
    def sendPOSI(self, lat:float, lon:float, elev:float, phi:float, theta:float, psi_true:float, ac:int=0) -> None:
        ...
        """
        if self.sim_mode is not None:
            self.sim_mode.sendPOSI(ac, lat, lon, elev, 0.0, 0.0, psi_true)
    ...
    def getPOSI(self) -> Tuple[float,float,float,float,float,float,float,float,float,float,float,float,float]:
        ...
        if self.sim_mode is not None:
            lat, lon, ele, y_agl, phi, theta, psi_true, vx, vy, vz, p, q, r = self.sim_mode.getPOSI()
    ...
    def pauseSIM(self, set_pause:bool) -> None:
        ...
        if set_pause:
            if self.sim_mode:
                self.sim_mode.dref_dict['sim/time/paused'] = True
            else:
                self.sendCMND('sim/operation/pause_on')
        else:
            if self.sim_mode:
                self.sim_mode.dref_dict['sim/time/paused'] = False
            else:
